...
# (Optional) get the current view for later Show/Render calls
view = GetActiveViewOrCreate('RenderView')

def require_source(name: str):
    """Return a pipeline source by name or raise a clear error."""
    src = FindSource(name)
    if src is None:
        raise RuntimeError(f"Pipeline item '{name}' was not found. "
                           f"Make sure it exists and the name matches exactly.")
    return src

# --- Retrieve your existing items by name ---
Connectivity1 = require_source("Connectivity1")                # centerlines (post-clean & connectivity)
XMLUnstructuredGridReader1 = require_source("XMLUnstructuredGridReader1")  # results.vtu reader
XMLPolyDataReader2 = require_source("XMLPolyDataReader2")      # results.vtp reader 

# -------------------------
# 4) Programmable Filter 2 on the connected centerlines
# -------------------------
pf2 = ProgrammableFilter(Input=[Connectivity1])
...
# -------------------------
grp = GroupDatasets(Input=[pf3, XMLUnstructuredGridReader1])
grp.UpdatePipeline()
...
# -------------------------
pf4 = ProgrammableFilter(Input=[grp])
pf4.Script = r"""
...
grp = GroupDatasets(Input=[cellToPoint, XMLPolyDataReader2])
grp.UpdatePipeline()
...
# -------------------------
pf5 = ProgrammableFilter(Input=[grp])
pf5.Script = r"""
import vtk
from vtkmodules.util import numpy_support
import numpy as np
...
grp = GroupDatasets(Input=[pf5, XMLUnstructuredGridReader1])
grp.UpdatePipeline()
...
# -------------------------
pf6 = ProgrammableFilter(Input=[grp])
pf6.Script = r"""
