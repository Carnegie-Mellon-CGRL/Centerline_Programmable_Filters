...
If you want to resample the data points and apply additional arrays (Generations, Resistance, Sidedness, etc), then apply the script Resample_Run to the results of WALT_Run:
 ...
 - Filter 13: “Flow_Power_and_Resistance” (Calculates Power at each point as a pressure drop between the current point and the previous point times the flow at that point. Calculates Ohm’s Law Resistance as the pressure drop divided by the flow)
 - Group with vtu and apply Filter 14: “True_Resistance” (Calculates Poiseuille Law Resistance as R = 8*μ L / (π r^4), where μ = a blood viscosity of 4.0 cP, L = 0.01 cm, and r is determined as the average radius from a slice at each point)
